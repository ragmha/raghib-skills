...
- The user mentions a specific Copilot CLI model (GPT-5.5, Claude Sonnet, Claude Opus, etc.) and wants pricing.
...
1. Identify the model (ask which GPT model if ambiguous).
2. Run:
   ```bash
   node scripts/cost.mjs estimate <model> --in <prompt_tokens> [--cached-in N] [--cache-write N] --out <expected_out>
   ```
